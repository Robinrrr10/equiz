...
public class QuestionDBHelper {
	...
	public void addQuestion(String question, String tag, String level) throws SQLException, ClassNotFoundException {
		String sqlQuery = "INSERT INTO questions (question, tag, level, is_used) VALUES (?, ?, ?, ?);";
		PreparedStatement preparedStatement = DBUtils.getConnection().prepareStatement(sqlQuery);
	...
	public List<QuestionEntry> getQuizQuestionsBasedOnTag(int noOfQuestionsInEachTag) throws SQLException, ClassNotFoundException {
		List<QuestionEntry> questionEntries = new ArrayList<QuestionEntry>();
		List<Integer> questionIds = new ArrayList<Integer>();
		for(Tag tag : Tag.values()) {
		String sqlQuery = "SELECT * FROM questions WHERE is_used='notused' AND tag='"+tag.name()+"' LIMIT "+noOfQuestionsInEachTag+";";
		PreparedStatement preparedStatement = DBUtils.getConnection().prepareStatement(sqlQuery);
		ResultSet resultSet = DBUtils.fetch(preparedStatement);
		while(resultSet.next()) {
			QuestionEntry questionEntry = new QuestionEntry();
	...
	public List<QuestionEntry> getQuizQuestionsBasedOnLevel(int noOfQuestionsInEachLevel) throws SQLException, ClassNotFoundException {
		List<QuestionEntry> questionEntries = new ArrayList<QuestionEntry>();
		List<Integer> questionIds = new ArrayList<Integer>();
		for(Level level : Level.values()) {
		String sqlQuery = "SELECT * FROM questions WHERE is_used='notused' AND level='"+level.name().toLowerCase()+"' LIMIT "+noOfQuestionsInEachLevel+";";
		PreparedStatement preparedStatement = DBUtils.getConnection().prepareStatement(sqlQuery);
		ResultSet resultSet = DBUtils.fetch(preparedStatement);
		while(resultSet.next()) {
			QuestionEntry questionEntry = new QuestionEntry();
	...
	public void updateQuestionUsed(List<Integer> questionIds, String isUsed) throws SQLException, ClassNotFoundException {
